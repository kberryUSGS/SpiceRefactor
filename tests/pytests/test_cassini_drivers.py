...
def usgscsm_compare_dict():
    return {'radii': 
            ...
            {'positions': [np.array([-2.30714008e+11, -1.39662228e+12,  3.09872114e+11])], 
             'velocities': [np.array([-7.41139016e+07,  1.22563268e+07,  3.85603215e+03])], 'unit': 'm'}, 
            'sensor_orientation': 
...
class test_cassini_pds3_naif(unittest.TestCase):
    ...
    def test_focal2pixel_samples(self):
        with patch('ale.drivers.co_drivers.spice.gdpool', return_value=[10.0]) as gdpool, \
             ...
             assert self.driver.focal2pixel_samples == [0.0, 100.0, 0.0]
             gdpool.assert_called_with('INS-12345_PIXEL_SIZE', 0, 1)
             
    def test_focal2pixel_lines(self):
        with patch('ale.drivers.co_drivers.spice.gdpool', return_value=[10.0]) as gdpool, \
             patch('ale.base.data_naif.spice.bods2c', return_value=-12345) as bods2c:
             assert self.driver.focal2pixel_lines == [0.0, 0.0, 100.0]
             gdpool.assert_called_with('INS-12345_PIXEL_SIZE', 0, 1)
    ...
    def test_focal_epsilon(self):
        with patch('ale.drivers.co_drivers.spice.gdpool', return_value=[10.0]) as gdpool, \
             patch('ale.base.data_naif.spice.bods2c', return_value=-12345) as bods2c:
             assert self.driver.focal_epsilon == 10
    ...
    def test_detector_center_sample(self):
        with patch('ale.drivers.co_drivers.spice.gdpool', return_value=[10.0]) as gdpool, \
             patch('ale.base.data_naif.spice.bods2c', return_value=-12345) as bods2c:
             assert self.driver.detector_center_sample == 10
...
#        with patch('ale.drivers.co_drivers.spice.gdpool', return_value=[10.0]) as gdpool, \
#             patch('ale.base.data_naif.spice.bods2c', return_value=-12345) as bods2c:
#             assert self.driver.frame_chain == 10
#             gdpool.assert_called_with('INS-12345_FOV_CENTER_PIXEL', 0, 2)
